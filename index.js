...
class Monster {
    ...
    show() {
        ...
        if (this.roar != "NA") {
            roarAudio = new Audio('Monster Roar/' + this.roar[0] + '/' + this.roar + '.mp3');
        } else {
            roarAudio = new Audio("Monster Theme/B/button-3.mp3");
        }
    }
    smashing(smash) {
        if (this.smash != smash) {
    ...
    if (inSplash) {
        ...
        if (canvas.getContext) {
            ...
            bg.onload = () => {
                ...
                ctx.font = "36px serif";
                ctx.fillText("Monster Hunter: Smash or Pass", 65, 80);
                ctx.font = "20px serif";
                ctx.fillText("Made by: mjr117 and Benwor", 65, 100);
                ...
                ctx.font = "25px serif";
                ctx.fillText("Play intro audio", 230, 540);

                ctx.fillStyle = 'black';
                ctx.font = "16px serif";
                printAtWordWrap(ctx, introTextL1, 30, 150, 25, 540);
                printAtWordWrap(ctx, introTextL2, 30, 180, 25, 540);
            }
            bg.src = bgSource;
        }
    } else {
        if (!initalSetup) {
            ...
            canvas.addEventListener('click', function (evt) {
                ...
                } else if (isInside(mousePos, roarButton)) {
                    themeAudio.pause(); themeAudio.currentTime = 0;
                    themeAudio.volume = audioVolume;
        ...
        if (canvas.getContext) {
            ...
            bg.onload = () => {
                    ...
                    ctx.font = "20px serif";
                    ctx.fillText(titleText, 30, 120);
    
                    // Current Count text
                    ctx.font = "40px serif";
                    ctx.fillText(smashCount + " : " + leftCount + " : " + passCount, 200, 540);
                    //console.log(smashCount,leftCount,passCount);
    
                    // Description text for the monster
                    ctx.font = "16px serif";
                    printAtWordWrap(ctx, descText, 30, 370, 25, 540);
                    ...
                    ctx.fillStyle = 'black';
                    ctx.fillRect(roarButton.x, roarButton.y, roarButton.width, roarButton.height);
                    // The text for the buttons
                    ctx.fillStyle = '#f5bd05';
                    ctx.font = "20px serif";
                    ctx.fillText("Theme", 70, 535);
                    ctx.fillStyle = 'white';
                    ctx.font = "20px serif";
                    ctx.fillText("Go back", 465, 535);
                    ctx.save();
                    ctx.rotate(Math.PI / 2);
                    ctx.fillStyle = 'black';
                    ctx.font = "20px serif";
                    ctx.fillText("SMASH", 190, -50);
                    ctx.restore();
                    ctx.save();
                    ctx.rotate(-Math.PI / 2);
                    ctx.fillStyle = 'black';
                    ctx.font = "20px serif";
                    ctx.fillText("PASS", -250, 550);
                    ctx.restore();
                    ctx.fillStyle = '#f5bd05';
                    ctx.font = "20px serif";
                    ctx.fillText("Roar", 135, 535);
            }
            bg.src = bgSource;
        }
    }
